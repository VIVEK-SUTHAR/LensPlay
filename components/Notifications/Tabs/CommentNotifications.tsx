...
function CommentNotifications() {
	...
	const LensNotificationTypes = [NotificationType.Commented];
	...
	const notifications = data?.notifications?.items as NewCommentNotification[];
	...
	const onEndCallBack = () => {
		console.log("Comment",pageInfo?.next);
		if (!pageInfo?.next) {
			return;
		}
		fetchMore({
	...
	const keyExtractor = (item: NewCommentNotification, index: any) => `${item?.id}-${index}`;
