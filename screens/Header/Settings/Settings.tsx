...
const Settings: React.FC<RootStackScreenProps<"Settings">> = ({ navigation }) => {
	const [, setIsReadyToRender] = React.useState(false);
	...
	const SettingItemsList: SettingsItemProps[] = [
		...
		},
		{
			...
			onPress: () => {
				void Linking.openURL(LENSPLAY_PRIVACY);
			},
		},
		{
			...
			label: "Contact Us",
			onPress: () => {
				void Linking.openURL(`mailto:${OFFICIAL_EMAIL}`);
			},
		},
	];
		...
		navigation.setOptions({
			headerRight: () => {
				return (
					<Pressable
						onPress={() => {
							navigation.push("ProfileScanner");
						}}
					>
