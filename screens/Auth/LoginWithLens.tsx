...
import { useAuthenticateMutation, useChallengeLazyQuery } from "../../types/generated";
...
const LoginWithLens: FC<RootStackScreenProps<"LoginWithLens">> = ({ navigation }) => {
	...
	const shortenAddress = (address: string): string => {
		if (address.length === 0) return "0X...000";
		return "0x.." + address.slice(address.length - 4, address.length);
	};

	const handleLoginWithLens = async (): Promise<void> => {
		try {
			setIsloading(true);
			...
			<LinearGradient colors={["#2D3436", "black", "#000000"]} style={{ flex: 1 }}>
				...
				>
					...
					<View>
						{!isDesktop ? (
							...
							>
								...
								>
									...
									>
										<Heading
											title={currentProfile?.name != null || shortenAddress(connector?.accounts[0])}
											style={{
												color: "white",
												fontSize: 16,
												fontWeight: "500",
						...
						>
							<Button
								title={hasHandle === true ? "Login with Lens" : "Claim test handle"}
								isLoading={isloading}
								...
								onPress={async () => {
									if (isDesktop) {
										void Linking.openURL("https://claim.lens.xyz/");
									} else {
