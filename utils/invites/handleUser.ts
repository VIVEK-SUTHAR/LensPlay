...
export default async function handleUser(profileId: string): Promise<boolean | undefined> {
	try {
		// isAlready user or not

		const isUserRes = await fetch("https://lensplay-api.vercel.app/api/user/checkUser", {
			method: "POST",
			headers: {
