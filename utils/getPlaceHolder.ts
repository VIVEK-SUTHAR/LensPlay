...
function getPlaceHolderImage(avatar?: boolean) {
  ...
  const AvatarImagePlaceHolder = [
    "LbN0;wfQuPoftSj[V@ayOsj[s8ay",
    "LIGRk.j[01j[eTfQxuay0Mj[~Bay",
  ];
  let Placeholder = "";
  if (avatar) {
    Placeholder =
      AvatarImagePlaceHolder[
        Math.floor(Math.random() * AvatarImagePlaceHolder.length)
      ];
  }
  Placeholder =
